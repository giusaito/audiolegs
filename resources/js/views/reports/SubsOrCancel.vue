<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" style="width: 100%" :default-sort="{prop: 'data', order: 'descending'}">
      ...
      <el-table-column align="center" label="Assinaturas" prop="qtd_ass" sortable>
        <template slot-scope="{row}">
          <span>{{ row.qtd_ass }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="Cancelamentos" prop="qtd_cancel" sortable>
        <template slot-scope="{row}">
...
<script>
import { fetchSubsOrCancel } from '@/api/reports';
...
export default {
  ...
  methods: {
    ...
    async getLaws() {
      ...
      const { data } = await fetchSubsOrCancel(this.listQuery);
